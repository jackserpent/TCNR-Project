...
if($data !="")
{
    ...
    if(isset($mydata["ID"]) && isset($mydata["Active"]) && $mydata["ID"] != "" && $mydata["Active"] != "")
    {
        ...
        // 更新 book_member 資料表中的 Actve 欄位
        $sql_update = "UPDATE book_member SET Active = '$m_Active' WHERE ID = '$m_ID'";

        if(mysqli_query($conn, $sql_update))
        {
            $sql_check = "SELECT mau.* FROM member_active_update_date AS mau JOIN book_member AS bm ON mau.UserId = bm.ID WHERE bm.ID = '$m_ID'";

            $result = mysqli_query($conn, $sql_check);

            // 檢查member_active_update_date資料表內是否已經存在相對應UserId的紀錄
            if(mysqli_num_rows($result) > 0)
            {
                // 如果已經存在對應UserId的紀錄，則更新的 Update_at 欄位
                $sql_update_time = "UPDATE member_active_update_date SET Update_at = CURRENT_TIMESTAMP WHERE UserId = '$m_ID'";

                if(mysqli_query($conn, $sql_update_time))
                {
                    //更新成功
                    echo '{"state":true,"message":"停權狀態與時間更新成功!"}';
                }else{
                    //更新失敗
                ...
                $sql_insert = "INSERT INTO member_active_update_date (UserId, Update_at) VALUES ('$m_ID', CURRENT_TIMESTAMP)";

                if(mysqli_query($conn, $sql_insert))
                {
                    //更新成功
                    echo '{"state":true,"message":"停權狀態更新成功與updata時間新增成功!"}';
                }else{
                    //更新失敗
                    echo '{"state":false,"message":"更新失敗!'.$sql.mysqli_error($conn).'"}';
                }
