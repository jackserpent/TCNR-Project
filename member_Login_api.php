...
if($data !="")
{
    ...
    if(isset($mydata["UserName"]) && isset($mydata["Password"]) && $mydata["UserName"] !="" && $mydata["Password"] !="")
    {
        ...
        $sql = "SELECT UserName, Password, Active FROM book_member WHERE UserName = '$m_UserName'";
        ...
        if(mysqli_num_rows($result) == 1)
        {
            ...
            if($row["Active"] == 1 )
            {
                ...
                //Active不等於1，也就是被停權狀態，登入失敗
                echo '{"state":false,"message":"您已被停權，請洽管理員!"}';
            }    
        }else{
